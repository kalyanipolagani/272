...
function addFeaturesToMultiLoc(map, name, addresses, cords, totalDist, truckReg) {
...
function initiate(nameArr, addressArr, truckReg) {
	...
	for (var index = 0; index < addressArr.length; index++) {
		getCoordinates(addressArr[index], function(latLng) {
			...
			if (counter == addressArr.length) {
				...
				addFeaturesToMultiLoc(map, nameArr, addressArr, addressCords, totalDist, truckReg);
			}
		});
	}
}